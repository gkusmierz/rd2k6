...
- Parse conversation history for `/blah:spec-impl <feature> [tasks]` commands
...
- If no history found, scan `.blah/specs/` for features with completed tasks `[x]`

**If feature provided** (feature specified, tasks empty):
- Use specified feature
- Detect all completed tasks `[x]` in `.blah/specs/{feature}/tasks.md`
...
- Read `.blah/specs/<feature>/spec.json` for metadata
- Read `.blah/specs/<feature>/requirements.md` for requirements
- Read `.blah/specs/<feature>/design.md` for design structure
- Read `.blah/specs/<feature>/tasks.md` for task list
- **Load ALL steering context**: Read entire `.blah/steering/` directory including:
...
## Tool Guidance
- **Conversation parsing**: Extract `/blah:spec-impl` patterns from history
...
### Error Scenarios
- **No Implementation Found**: If no `/blah:spec-impl` in history and no `[x]` tasks, report "No implementations detected"
