...
- Glob(`.blah/settings/templates/steering-custom/*.md`) to find available templates
...
2. **Check if template exists**:
   - Load from `.blah/settings/templates/steering-custom/{name}.md` if available
...
4. **Generate custom steering**:
   ...
   - Apply principles from `.blah/settings/rules/steering-principles.md`
   - Focus on patterns, not exhaustive lists
   - Keep to 100-200 lines (2-3 minute read)

5. **Create file** in `.blah/steering/{name}.md`

## Available Templates

Templates available in `.blah/settings/templates/steering-custom/`:
...
## Created:
- .blah/steering/api-standards.md
