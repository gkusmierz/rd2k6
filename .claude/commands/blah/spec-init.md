...
## Execution Steps
1. **Check Uniqueness**: Verify `.blah/specs/` for naming conflicts (append number suffix if needed)
2. **Create Directory**: `.blah/specs/[feature-name]/`
3. **Initialize Files Using Templates**:
   - Read `.blah/settings/templates/specs/init.json`
   - Read `.blah/settings/templates/specs/requirements-init.md`
...
4. **Next Step**: Command block showing `/blah:spec-requirements <feature-name>`
...
- **Template Missing**: If template files don't exist in `.blah/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
