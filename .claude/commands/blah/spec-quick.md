...
2. **Check Uniqueness**:
   - Use Glob to check `.blah/specs/*/`
   - If feature name exists, append `-2`, `-3`, etc.

3. **Create Directory**:
   - Use Bash: `mkdir -p .blah/specs/{feature-name}`
...
4. **Initialize Files from Templates**:
   ...
   - .blah/settings/templates/specs/requirements-init.md
   ```
   ...
   ```
   - .blah/specs/{feature-name}/spec.json
   - .blah/specs/{feature-name}/requirements.md
   ```
...
6. **Output Progress**:
   ```
   ✅ Spec initialized at .blah/specs/{feature-name}/
   ```
...
```
/blah:spec-requirements {feature-name}
```
...
**Execute SlashCommand**:
```
/blah:spec-design {feature-name} -y
```
...
**Execute SlashCommand**:
```
/blah:spec-tasks {feature-name} -y
```
...
### Phase 1 Tools
- **Glob**: Check `.blah/specs/*/` for existing feature names
- **Bash**: Create directory with `mkdir -p`, generate timestamp with `date -u`
- **Read**: Fetch templates from `.blah/settings/templates/specs/`
- **Write**: Create `spec.json` and `requirements.md` in spec directory

### Phase 2-4 Tools
- **SlashCommand**: Execute `/blah:spec-requirements`, `/blah:spec-design`, `/blah:spec-tasks`
...
```
✅ Spec initialized at .blah/specs/{feature}/
✅ Requirements generated → Continuing to design...
✅ Design generated → Continuing to tasks...
```
...
## Generated Files:
- .blah/specs/{feature}/spec.json
- .blah/specs/{feature}/requirements.md ({X} requirements)
- .blah/specs/{feature}/design.md ({Y} components, {Z} endpoints)
- .blah/specs/{feature}/tasks.md ({N} tasks)

⚠️ Quick generation skipped:
- `/blah:validate-gap` - Gap analysis (integration check)
- `/blah:validate-design` - Design review (architecture validation)

## Next Steps:
1. Review generated specs (especially design.md)
2. Optional validation:
   - `/blah:validate-gap {feature}` - Check integration with existing codebase
   - `/blah:validate-design {feature}` - Verify architecture quality
3. Start implementation: `/blah:spec-impl {feature}`

## Note:
For complex features (integrations, security, APIs), use standard workflow:
/blah:spec-init → /blah:spec-requirements → /blah:validate-gap
→ /blah:spec-design → /blah:validate-design → /blah:spec-tasks
```
...
**Template Missing**:
- Check `.blah/settings/templates/specs/` exists
...
- Suggest: "Continue manually from `/blah:spec-{next-phase} {feature}`"
