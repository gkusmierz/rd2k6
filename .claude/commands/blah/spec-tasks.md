...
- Verify `.blah/specs/$1/` exists
- Verify `.blah/specs/$1/design.md` exists
...
Task(
  ...
  prompt="""
Feature: $1
Spec directory: .blah/specs/$1/
Auto-approve: {true if $2 == "-y", else false}
Sequential mode: {true if sequential else false}

File patterns to read:
- .blah/specs/$1/*.{json,md}
- .blah/steering/*.md
- .blah/settings/rules/tasks-generation.md
- .blah/settings/rules/tasks-parallel-analysis.md (include only when sequential mode is false)
- .blah/settings/templates/specs/tasks.md
...
- **IMPORTANT**: Clear conversation history and free up context before running `/blah:spec-impl`
- This applies when starting first task OR switching between tasks
- Fresh context ensures clean state and proper task focus

**If Tasks Approved**:
- Execute specific task: `/blah:spec-impl $1 1.1` (recommended: clear context between each task)
- Execute multiple tasks: `/blah:spec-impl $1 1.1,1.2` (use cautiously, clear context between tasks)
- Without arguments: `/blah:spec-impl $1` (executes all pending tasks - NOT recommended due to context bloat)

**If Modifications Needed**:
- Provide feedback and re-run `/blah:spec-tasks $1`
