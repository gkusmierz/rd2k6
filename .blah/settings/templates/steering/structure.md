...
- **Files**: `snake_case.h`, `snake_case.cpp` (follows Qt convention for custom code)
...
The domain library MUST compile without Qt in its link dependencies (`QT -= core gui` in its `.pro` file). This enforces domain purity at the build system level.

## Code Organization Principles

- **Dependency rule**: domain ← ports ← adapters ← app. Never the reverse.
- **Domain purity**: `src/domain/` compiles with pure C++ standard library only. No `#include <Q*>`.
- **One class per file**: Each `.h`/`.cpp` pair contains one primary class.
- **Header guards**: `#pragma once` (supported by all target compilers).
